...
class AddPostViewModel extends ChangeNotifier {
  ...
  AddPostViewModel(this.user, this._document) {
    ...
    if (this._document == null) {
      ...
      viewDate =
          Timestamp.fromDate(this._document['diary_date'].toDate()); // 現在の日時
    }
  ...
  void postDiary(User user, Timestamp postedDate) async {
    ...
    debugPrint("postedDate : " +
        DateFormat('yyyy/MM/dd HH:mm').format(postedDate.toDate()));
    logger.v("Verbose log");
    await FirebaseFirestore.instance
        .collection('diary')
        .doc(DateFormat('yyyyMMdd').format(tPostedDate.toDate()) + user.uid)
        .set({
      'user_uid': user.uid,
